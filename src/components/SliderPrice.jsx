...
export default function SliderPrice({ onChange, initialMin, initialMax }) {
  const MIN = 20;
  const MAX = 10000;

  const [min, setMin] = useState(Number(initialMin) || MIN);
  const [max, setMax] = useState(Number(initialMax) || MAX);
  ...
  useEffect(() => {
    setMin(Number(initialMin) || MIN);
    setMax(Number(initialMax) || MAX);
